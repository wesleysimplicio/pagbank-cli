...
CLI para PagBank — pagamentos, carteira e serviços PagSeguro
...
2. Use a skill **`pagbank-br`** para entender o vocabulário e os fluxos esperados.
...
- **Nome do pacote:** `pagbank-cli`
- **Comando instalado:** `pagbank`
- **Runtime:** Python 3.9+
- **API base:** `https://api.pagseguro.com.br/`
- **Documentação da API:** https://dev.pagseguro.uol.com.br/
- **Endpoints mapeados:** variam por serviço
- **Auth:** OAuth 2.0 / Client Credentials (padrão Open Finance / bancos)
- **Cache local de sessão:** `~/.hermes2/scripts/pagbank-cli/`
...
1. Leia `AGENTS.md` antes de editar qualquer coisa.
2. Use a skill **`pagbank-br`** para entender os fluxos esperados.
...
```bash
git clone https://github.com/wesleysimplicio/pagbank-cli.git
cd pagbank-cli
pip install -e .
```
...
| Comando | O que faz |
|---|---|
| `pagbank --help` | lista os comandos disponíveis |
| `pagbank config set` | define base URLs, certificados e credenciais locais |
| `pagbank config show` | mostra a configuração salva |
| `pagbank config path` | mostra o arquivo de configuração usado pelo CLI |
...
A skill `pagbank-br` permite que o Hermes execute ações como:
...
```bash
claude -p "Rode pagbank --help e me diga o que está disponível"
codex -p "Rode pagbank config show e resuma a configuração"
```
